...
TEST(probe_go_functions) {
    NcpMetrics m = ncp_metrics("svc.go",
        "package svc\n\n"
...
TEST(probe_go_calls_edge) {
    NcpMetrics m = ncp_metrics("calc.go",
        "package calc\n\n"
...
TEST(probe_go_variadic_named_return) {
    NcpMetrics m = ncp_metrics("util.go",
        "package util\n\n"
...
TEST(probe_go_interface_node) {
    NcpMetrics m = ncp_metrics("iface.go",
        "package iface\n\n"
...
TEST(probe_go_closure) {
    NcpMetrics m = ncp_metrics("cb.go",
        "package cb\n\n"
...
TEST(probe_go_generic_function) {
    NcpMetrics m = ncp_metrics("gen.go",
        "package gen\n\n"
...
TEST(probe_c_functions) {
    NcpMetrics m = ncp_metrics("math.c",
...
TEST(probe_c_calls_edge) {
    NcpMetrics m = ncp_metrics("proc.c",
...
TEST(probe_c_inline_function) {
    NcpMetrics m = ncp_metrics("inl.c",
...
TEST(probe_c_struct_node) {
    NcpMetrics m = ncp_metrics("point.c",
...
TEST(probe_cpp_class_methods) {
    NcpMetrics m = ncp_metrics("counter.cpp",
        "#include <cstdio>\n"
        "class Counter {\n"
        "    int n;\n"
        "public:\n"
...
TEST(probe_cpp_calls_edge) {
    NcpMetrics m = ncp_metrics("calc.cpp",
...
TEST(probe_cpp_template_function) {
    NcpMetrics m = ncp_metrics("tpl.cpp",
        "template <typename T>\n"
...
TEST(probe_cpp_enum_class) {
    NcpMetrics m = ncp_metrics("color.cpp",
...
TEST(probe_cpp_struct_operator) {
    NcpMetrics m = ncp_metrics("vec.cpp",
...
TEST(probe_cpp_inheritance) {
    NcpMetrics m = ncp_metrics("shape.cpp",
        "class Shape {\n"
        "public:\n"
...
TEST(probe_rust_struct_impl) {
    NcpMetrics m = ncp_metrics("stack.rs",
...
TEST(probe_rust_trait_impl) {
    NcpMetrics m = ncp_metrics("greet.rs",
        "trait Greet {\n"
...
TEST(probe_rust_calls_edge) {
    NcpMetrics m = ncp_metrics("math.rs",
...
TEST(probe_rust_closure) {
    NcpMetrics m = ncp_metrics("fmap.rs",
        "fn double_all(v: &[i32]) -> Vec<i32> {\n"
...
TEST(probe_rust_type_alias) {
    NcpMetrics m = ncp_metrics("alias.rs",
        "type Meters = f64;\n"
        "type Seconds = f64;\n\n"
...
TEST(probe_python_functions) {
    NcpMetrics m = ncp_metrics("math.py",
        "def add(a, b):\n    return a + b\n\n\n"
        "def mul(a, b):\n    return a * b\n\n\n"
...
TEST(probe_python_class) {
    NcpMetrics m = ncp_metrics("model.py",
        "class Account:\n"
...
TEST(probe_python_nested_function) {
    NcpMetrics m = ncp_metrics("closure.py",
...
TEST(probe_python_decorator) {
    NcpMetrics m = ncp_metrics("deco.py",
        "def log_call(fn):\n"
...
TEST(probe_python_lambda) {
    NcpMetrics m = ncp_metrics("lmb.py",
        "double = lambda x: x * 2\n\n\n"
        "def apply(fn, items):\n"
...
TEST(probe_python_dataclass) {
    NcpMetrics m = ncp_metrics("point.py",
...
TEST(probe_ts_interface_class) {
    NcpMetrics m = ncp_metrics("iface.ts",
        "interface Shape {\n"
        "    area(): number;\n"
        "    perimeter(): number;\n"
        "}\n\n"
...
TEST(probe_ts_generic_arrow) {
    NcpMetrics m = ncp_metrics("util.ts",
...
TEST(probe_ts_enum) {
    NcpMetrics m = ncp_metrics("status.ts",
...
TEST(probe_ts_class_extends) {
    NcpMetrics m = ncp_metrics("vehicle.ts",
        "class Vehicle {\n"
...
TEST(probe_ts_type_alias) {
    NcpMetrics m = ncp_metrics("types.ts",
...
TEST(probe_java_class_methods) {
    NcpMetrics m = ncp_metrics("Calc.java",
        "package app;\n\n"
        "class Calc {\n"
        "    private int base;\n\n"
...
TEST(probe_java_enum) {
    NcpMetrics m = ncp_metrics("Day.java",
        "package app;\n\n"
...
TEST(probe_java_inner_class) {
    NcpMetrics m = ncp_metrics("Outer.java",
        "package app;\n\n"
        "class Outer {\n"
        "    private int x;\n"
...
TEST(probe_java_lambda) {
    NcpMetrics m = ncp_metrics("Streams.java",
        "package app;\n\n"
...
TEST(probe_kotlin_data_class) {
    NcpMetrics m = ncp_metrics("User.kt",
...
TEST(probe_kotlin_sealed_class) {
    NcpMetrics m = ncp_metrics("Result.kt",
...
TEST(probe_kotlin_interface_lambda) {
    NcpMetrics m = ncp_metrics("Filter.kt",
        ...
        "    filterList(xs, object : Predicate<Int> { override fun test(item: Int) = item % 2 == 0 })\n");
    ASSERT_TRUE(m.ok);
    ASSERT_TRUE(m.types >= 1);
    ASSERT_TRUE(m.functions >= 1);
    PASS();
...
TEST(probe_kotlin_calls_edge) {
    NcpMetrics m = ncp_metrics("Math.kt",
...
TEST(probe_kotlin_enum) {
    NcpMetrics m = ncp_metrics("Color.kt",
...
TEST(probe_cs_class_methods) {
    NcpMetrics m = ncp_metrics("Calc.cs",
        "namespace App {\n"
        "    class Calc {\n"
        "        private int offset;\n"
...
TEST(probe_cs_interface) {
    NcpMetrics m = ncp_metrics("IRepo.cs",
        "namespace App {\n"
        "    interface IRepo<T> {\n"
        "        T Get(int id);\n"
        "        void Save(T item);\n"
...
/* C#: enum type. */
TEST(probe_cs_enum) {
    NcpMetrics m = ncp_metrics("Status.cs",
        "namespace App {\n"
...
TEST(probe_cs_generic_class) {
    NcpMetrics m = ncp_metrics("Box.cs",
        "namespace App {\n"
        "    class Box<T> {\n"
        "        private T value;\n"
...
TEST(probe_cs_struct) {
    NcpMetrics m = ncp_metrics("Point.cs",
        "namespace App {\n"
        "    struct Point {\n"
        "        public float X, Y;\n"
...
TEST(probe_ruby_class_calls) {
    NcpMetrics m = ncp_metrics("greeter.rb",
        "class Greeter\n"
        "  def initialize(name)\n"
        "    @name = name\n"
        "  end\n\n"
...
TEST(probe_ruby_module) {
    NcpMetrics m = ncp_metrics("utils.rb",
        "module MathUtils\n"
        "  def self.square(x)\n"
        "    x * x\n"
        "  end\n\n"
...
TEST(probe_ruby_toplevel_functions) {
    NcpMetrics m = ncp_metrics("calc.rb",
        "def add(a, b)\n  a + b\nend\n\n"
        "def mul(a, b)\n  a * b\nend\n\n"
        "def run\n  add(mul(2, 3), 1)\nend\n");
    ASSERT_TRUE(m.ok);
...
TEST(probe_swift_struct_calls) {
    NcpMetrics m = ncp_metrics("geo.swift",
        "struct Point {\n"
        "    var x: Double\n"
        "    var y: Double\n\n"
...
TEST(probe_swift_class_inherit) {
    NcpMetrics m = ncp_metrics("shape.swift",
        "class Shape {\n"
...
TEST(probe_swift_enum) {
    NcpMetrics m = ncp_metrics("result.swift",
        "enum AppError: Error {\n"
...
TEST(probe_scala_class_object) {
    NcpMetrics m = ncp_metrics("calc.scala",
        "class Calc(base: Int) {\n"
...
TEST(probe_scala_trait) {
    NcpMetrics m = ncp_metrics("greet.scala",
        "trait Greeter {\n"
...
TEST(probe_lua_functions_calls) {
    NcpMetrics m = ncp_metrics("math.lua",
...
TEST(probe_lua_table_class) {
    NcpMetrics m = ncp_metrics("counter.lua",
        "local Counter = {}\n"
...
TEST(probe_bash_functions_calls) {
    NcpMetrics m = ncp_metrics("deploy.sh",
        "#!/usr/bin/env bash\n\n"
        "log() {\n"
        "    echo \"[INFO] $*\"\n"
        "}\n\n"
...
TEST(probe_bash_function_keyword) {
    NcpMetrics m = ncp_metrics("lib.sh",
        "function helper {\n"
        "    echo \"help: $1\"\n"
        "}\n\n"
        "function runner {\n"
...
TEST(probe_dart_functions) {
    NcpMetrics m = ncp_metrics("math.dart",
...
TEST(probe_dart_class) {
    NcpMetrics m = ncp_metrics("account.dart",
        "class Account {\n"
        "    String owner;\n"
        "    double balance;\n\n"
...
TEST(probe_dart_calls_edge_known_gap) {
    NcpMetrics m = ncp_metrics("chain.dart",
        "void helper() {\n  print('helper');\n}\n\n"
        "void run() {\n  helper();\n}\n");
    ASSERT_TRUE(m.ok);
...
TEST(probe_elixir_module_calls) {
    NcpMetrics m = ncp_metrics("calc.ex",
        "defmodule Calc do\n"
        "  def add(a, b), do: a + b\n\n"
        "  def mul(a, b), do: a * b\n\n"
...
TEST(probe_elixir_nested_module) {
    NcpMetrics m = ncp_metrics("server.ex",
...
TEST(probe_haskell_functions_calls) {
    NcpMetrics m = ncp_metrics("math.hs",
        "module Math where\n\n"
        "square :: Int -> Int\n"
        "square x = x * x\n\n"
        "cube :: Int -> Int\n"
...
TEST(probe_haskell_data_type) {
    NcpMetrics m = ncp_metrics("tree.hs",
        "module Tree where\n\n"
...
TEST(probe_zig_functions_calls) {
    NcpMetrics m = ncp_metrics("math.zig",
...
TEST(probe_zig_struct_methods) {
    NcpMetrics m = ncp_metrics("stack.zig",
        "const Stack = struct {\n"
...
TEST(probe_ocaml_functions_calls) {
    NcpMetrics m = ncp_metrics("math.ml",
        "let square x = x * x\n\n"
...
TEST(probe_erlang_functions_calls) {
    NcpMetrics m = ncp_metrics("math.erl",
        "-module(math).\n"
...
TEST(probe_groovy_calls_edge_known_gap) {
    NcpMetrics m = ncp_metrics("funcs.groovy",
        "def helper() {\n    println 'helping'\n}\n\n"
        "def runner() {\n    helper()\n}\n");
    ASSERT_TRUE(m.ok);
...
/* ══════════════════════════════════════════════════════════════════
 * GROUP 22 — Nim (grammar-only)
 * Labels: Function:2, Module:1
 * ══════════════════════════════════════════════════════════════════ */

/* Nim: proc definitions + CALLS edge. */
TEST(probe_nim_functions_calls) {
    NcpMetrics m = ncp_metrics("math.nim",
        "proc square(x: int): int =\n"
        "  x * x\n\n"
        "proc sumSquares(a, b: int): int =\n"
        "  square(a) + square(b)\n\n"
        "proc main() =\n"
        "  echo sumSquares(3, 4)\n\n"
        "main()\n");
...
TEST(probe_gdscript_functions_calls) {
    NcpMetrics m = ncp_metrics("player.gd",
        "extends Node\n\n"
        "var health: int = 100\n\n"
...
TEST(probe_perl_functions_calls) {
    NcpMetrics m = ncp_metrics("math.pl",
        "#!/usr/bin/perl\n"
        "use strict;\n"
        "use warnings;\n\n"
        "sub square {\n"
...
TEST(probe_r_functions_calls) {
    NcpMetrics m = ncp_metrics("math.R",
        "square <- function(x) {\n"
        "  x * x\n"
        "}\n\n"
...
TEST(probe_fortran_functions_calls) {
    NcpMetrics m = ncp_metrics("math.f90",
...
SUITE(node_creation_probe) {
    ...
    /* Nim (grammar-only) */
    RUN_TEST(probe_nim_functions_calls);
